...
func newRESTMux(client *whatsmeow.Client, messageStore *MessageStore, port int, token string, allowedMediaRoots []string) *http.ServeMux {
	...
	registerDesktopHandlers(mux, auth, messageStore)
	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
		...
		if client.Store.ID == nil {
			...
			for evt := range qrChan {
				if evt.Event == "code" {
					writePairingState("waiting_for_scan", evt.Code)
					if !qrCodeShown {
					...
					writePairingState("connected", "")
					connected <- true
					break
				} else if evt.Event == "timeout" {
					writePairingState("expired", "")
